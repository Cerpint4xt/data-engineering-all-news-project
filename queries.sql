...
CREATE OR REPLACE EXTERNAL TABLE `all-news-project.all_news_data.external_table_all_news_data`
OPTIONS (
    FORMAT='PARQUET',
    URIS=['gs://all-news-bucket-project/all_news_data/*']
)


-- Create Materialized Table from the external Table

CREATE OR REPLACE TABLE `all-news-project.all_news_data.materialized_table_all_news_data`
AS (
    SELECT * FROM `all-news-project.all_news_data.external_table_all_news_data`
)


-- Create Partitioning `date` and clustering `publication`
CREATE OR REPLACE TABLE `all-news-project-419219.all_news_data.all_news_partitioned_table`
PARTITION BY date
CLUSTER BY publication as (
    SELECT * FROM `all-news-project-419219.all_news_data.materialized_table_all_news_data` 
)
